...
class MeleeList extends Component {
  ...
  render() {
          ...
          {this.state.list.map((character) => {
            return (
              <span className="character-span" key={character.Name}>
                ...
                >
                  <img
                    className={`character-list series-${character.series}`}
                    src={require(`../assets/img/characters/${character.Name.toLowerCase()
                      .replace(/\./g, "")
                      .replace(/ /g, "_")
                      .replace(/&/g, "and")}.jpg`)}
                    alt={character.Name}
    ...
    if (this.state.error !== undefined) {
      return <ImageHandler message={this.state.error} image={"error.png"} />;
    } else {
      return <ImageHandler />;
    }
  }
